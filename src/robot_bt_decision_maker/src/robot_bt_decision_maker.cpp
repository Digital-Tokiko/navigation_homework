...
DecisionMakerNode::DecisionMakerNode(std::string name) : Node(name) {
    ...
    blackboard_->set<bool>("is_game_started", is_game_started);

    std::vector<robot_msgs::msg::MapInfo> map_infos;
    int enemy_num = 0;
    double sentry_hp = 0.0;
    bool is_transfering = false;
    bool is_bullet_low = false;

    blackboard_->set<std::vector<robot_msgs::msg::MapInfo>>("map_infos", map_infos);
    blackboard_->set<int>("enemy_num", enemy_num);
    blackboard_->set<double>("sentry_hp", sentry_hp);
    blackboard_->set<bool>("is_transfering", is_transfering);
    blackboard_->set<bool>("is_bullet_low", is_bullet_low);
